...
This directory is the shared data root for the atmosphere and synthesis
packages. Set `PAYNE_ZERO_DATA_ROOT` to use another location.
...
The runtime arrays are distributed through Git LFS. `install.sh` verifies every
file against `runtime_data_manifest.json`, installs the two default initializer
checkpoints, and builds persistent caches. Set
`PAYNE_ZERO_INCLUDE_DIRECT_XH=1` to install the optional
direct-abundance checkpoint.
