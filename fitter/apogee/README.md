...
`fit_apogee_spectrum` and `payne-zero-fit-apogee` use the fast synthesis-from-labels workflow during the repeated search. They do not run the iterative atmosphere solver. Use `fitter.refine_with_physical_atmosphere` with a converged-model callback when the retained result must use a physically converged atmosphere.
